...
import org.joda.time.{LocalDate, LocalTime, DateTime, Instant, Duration => JodaDuration}
...
trait SeqParameterValue {
  ...
  def typeName(tpe: Type): String = {
    tpe match {
      ...
      case t if t =:= typeOf[Timestamp] => "timestamp"
      case t if t =:= typeOf[java.util.Date] => "timestamp"
      ...
      case t if t =:= typeOf[JodaDuration] => "interval"
      case t if t =:= typeOf[ScalaDuration] => "interval"
    ...
    def asJava: Array[AnyRef] = {
      value.map(
        _.map {
          case a: QArray[_] =>
            a.asJava
          case otherwise => QArray.box(otherwise.value)
        }.orNull
      ).toArray
    }
